...
public class ComputerorderDaoImpl extends HibernateDaoSupport implements
		ComputerorderDao {
	...
	public int countRow(String hql) {
		return getHibernateTemplate().find(this.basicComputerorderSql+" "+hql).size();
	}
	...
	public Computerorder selectById(int computerorderid) {
		final String sql = basicComputerorderSql + " where id= "
				+ computerorderid;
	...
	public List<ComputerorderFull> selFullByStatus(int orderstatus) {
		// String selunderwayordersql = "  where a.createuserid="+uid +
		// " and a.status in("+ComputerorderInfo.ComputerorderStatusAduitWait+","+ComputerorderInfo.ComputerorderStatusAduitReject+") order by a.createtime desc";
		String selbystatus = "";
		if (orderstatus == 0) {
			selbystatus = " order by a.createtime desc";
		} else {
			selbystatus = " where  a.status =" + orderstatus
					+ " order by a.createtime desc";
		}
	...
	public List<ComputerorderFull> selFullByStatus(int uid, int orderstatus) {
		// String selunderwayordersql = "  where a.createuserid="+uid +
		// " and a.status in("+ComputerorderInfo.ComputerorderStatusAduitWait+","+ComputerorderInfo.ComputerorderStatusAduitReject+") order by a.createtime desc";
		String selbystatus = " where a.createuserid=" + uid + " and a.status ="
				+ orderstatus + " order by a.createtime desc";
	...
	public List<Computerorder> selectComputerorderByConditionAndPage(
			String conditionSql, final Page page) {
		...
		List<ComputerorderFull> computerorderList = getHibernateTemplate()
				.executeFind(new HibernateCallback() {
					public Object doInHibernate(Session session)
						...
						// Query query =
						// session.createSQLQuery(sql).setResultTransformer(Transformers.aliasToBean(YaomingFull.class));
						Query query = session.createSQLQuery(sql);
	...
	public int deleteEntity(Integer computerorderId) {
		...
		// log.debug("正在删除");
		try {
			String hqlString = "update Computerorder as tb set tb.status = 0 where tb.id = "
					+ computerorderId;
	...
	public ComputerorderFull selectComputerorderFullById(Integer computerorderId) {
		final String sql = basicComputerorderFullSql + " where a.id = "
				+ computerorderId;
