...
public class ComputerorderActionUtil2 {
	
	
	
//	构建某一个型号，某一时段 某一天的可借数量         天的长度是可提前预约的天数（预约n天内的PC）
//	computeroderadvanceorderday为真实的computeroderadvanceorderday，不是展示的
	public static 	HashMap<Integer,HashMap<Integer,ArrayList<Integer>>> computermodelPeriodDayInfo(List<Computermodel> computermodelList,Date todayDate,Date orderStartDate,int currentPeriod , List<Borrowperiod> borrowperiodList,int computeroderadvanceorderday){
...
//				是否是当前
				boolean equal = DateUtil.getDateDay(todayDate).equals(DateUtil.getDateDay(orderStartDate));
				int todaynum = 0;
				if(equal){
					//对于今天过去的时间段处理
					todaynum = 0;
