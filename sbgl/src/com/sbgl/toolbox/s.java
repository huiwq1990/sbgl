...
import net.oschina.beans.Space;
...
/**
 * 会员空间过滤器，以实现如下效果
 * http://my.oschina.net/javayou -> /WEB-INF/myspace/index.vm
 * @author Winter Lau
 */
public class MySpaceFilter implements Filter {
	...
	private final static String[] static_ext = 
		{"js","css","jpg","png","gif",".html","ico","vm","swf"};
	...
	public void doFilter(ServletRequest req1, ServletResponse res1,
		...
		if (!ArrayUtils.contains(domains, server) || 
				(paths.length>0 && "action".equalsIgnoreCase(paths[0]))
